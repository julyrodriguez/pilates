...
export function ShiftAttendeesModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4 overflow-y-auto">
      <div className="bg-white dark:bg-[#1c0c1e] border border-rose-200 dark:border-rose-900/50 rounded-2xl p-6 max-w-xl w-full shadow-2xl animate-modal my-8">
        ...
        <div className="space-y-3 mb-6 max-h-[340px] overflow-y-auto pr-1">
          {activeBookings.length === 0 ? (
            <div className="text-center py-8 bg-slate-50 dark:bg-[#120713] rounded-xl border border-dashed border-rose-200/60 dark:border-rose-900/40">
              <Users className="w-8 h-8 text-rose-300 mx-auto mb-2 opacity-60" />
            ...
            activeBookings.map((b, idx) => (
              <div
                key={b.id}
                className="p-3.5 rounded-xl bg-slate-50 dark:bg-[#120713] border border-rose-200/50 dark:border-rose-900/30 flex items-center justify-between gap-3"
              >
                <div className="flex items-center gap-3">
                  <div className="w-7 h-7 rounded-full bg-rose-500/15 text-rose-600 dark:text-rose-300 font-bold text-xs flex items-center justify-center">
                    {idx + 1}
                  </div>
                  <div>
                    <div className="text-xs font-bold text-slate-800 dark:text-rose-50">
                      {b.clientName}
                    </div>
                    <div className="flex items-center gap-3 text-[11px] text-slate-500 dark:text-rose-300/70 mt-0.5">
                    ...
                    {b.notes && (
                      <div className="text-[10px] text-amber-600 dark:text-amber-300 mt-1 italic">
                        Nota: {b.notes}
                      </div>
                    )}
                  </div>
        ...
        {/* Cancelled Attendees count if any */}
        {cancelledBookings.length > 0 && (
          <div className="mb-4 text-xs text-slate-400 dark:text-rose-400/60 flex items-center gap-1">
        ...
        {/* Footer */}
        <div className="flex items-center justify-between pt-3 border-t border-rose-200/50 dark:border-rose-900/30">
          <button
            onClick={onClose}
          ...
          {activeBookings.length < shift.capacity && (
            <button
              ...
              className="px-4 py-2 text-xs font-bold btn-rose-primary"
            >
              + Anotar Alumno Aquí
            </button>
          )}
