...
export function NotificationBell() {
  ...
  // Marcar automáticamente todas como leídas siempre que se abra la campanita
  useEffect(() => {
  ...
  return (
    <div className="relative inline-block" ref={menuRef}>
      ...
      {/* Mobile Backdrop for full-screen focus & easy dismiss */}
      {isOpen && (
        <div
      ...
      {isOpen && (
        <div className="fixed sm:absolute inset-x-3 sm:inset-x-auto top-16 sm:top-full mt-0 sm:mt-2 sm:right-0 sm:left-auto lg:left-0 lg:right-auto sm:w-88 max-w-sm sm:max-w-none mx-auto sm:mx-0 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl z-[60] overflow-hidden animate-modal">
          {/* Header */}
          <div className="p-3 sm:p-3.5 bg-slate-50 dark:bg-slate-900/90 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-1">
              ...
              <button
                type="button"
                onClick={() => setIsOpen(false)}
                className="p-1 ml-1 rounded-lg text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-200/60 dark:hover:bg-slate-800 transition-colors cursor-pointer sm:hidden"
          ...
          {/* Windows Desktop Notifications Enable Prompt (shown only on screens >= sm) */}
          {desktopPermission === "default" && (
            <div className="hidden sm:flex px-3 py-2 bg-gradient-to-r from-indigo-50/90 to-purple-50/80 dark:from-indigo-950/60 dark:to-purple-950/40 border-b border-indigo-100 dark:border-indigo-900/40 items-center justify-between gap-2">
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-800/60 max-h-[65vh] sm:max-h-96 overflow-y-auto">
